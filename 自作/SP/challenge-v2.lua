...
function onCreatePost()--特殊なUI変更
	debugPrint('set healthGain 1')
	debugPrint('set healthLoss 1')
	setProperty('healthGain',1)
	setProperty('healthLoss',1)
	--強制Middle scroll
	if not middleScroll then
        	setPropertyFromGroup('opponentStrums', 0, 'x', -5000);-- -5000
		...
		--noteTweenAlpha('opp1a', 0, 0.5, 0.25, 'linear')
		--noteTweenAlpha('opp2a', 1, 0.5, 0.25, 'linear')
		--noteTweenAlpha('opp3a', 2, 0.5, 0.25, 'linear')
		--noteTweenAlpha('opp4a', 3, 0.5, 0.25, 'linear')
	...
	makeLuaText('limitText', 'RecoveryLimit', 1210, 900, 340)-- x0 y150
	setTextAlignment('limitText', 'left')
	setObjectCamera('limitText', UISET)
	--screenCenter('limitText', 'x')
	setTextSize('limitText', 20)
	setProperty('limitText.alpha', 1)
	addLuaText('limitText')

	makeLuaText('minihealthText', '',500,890,600);
	setTextAlignment("minihealthText", "left");
	setTextSize('minihealthText', 20);
	addLuaText('minihealthText')
	setProperty('minihealthText.color', getColorFromHex('FFFFFF'))--字体の色です
	setTextBorder('minihealthText','1',BorderColor)--縁取りの色です
	setTextFont('minihealthText',fontset)--使用フォントです
	setObjectCamera('minihealthText', UISET)
...
function onUpdate()--細かい設定
	 ...
	 setTextString('minihealthText', 'HP:'..math.floor((health*50)/1).. '%')
	...
	if Recoveryreft == 0 then --else not
		...
		if botPlay and math.floor((health*50)/1) < 5 then 
			Recoveryreft = Recoveryreft - 1
			--setProperty('health', getProperty('health') + 1.500);
			runTimer('Recovery',0.01,0)
			--debugPrint("Recovery complete")
	...
	setTextString('limitText', 'RecoveryLimit\n' ..Recoveryreft..'\nto SPACE Recovery!')

	if math.floor((health*50)/1) > 99 then
		cancelTimer('Recovery')
	end
end
